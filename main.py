...
async def ingest(file: UploadFile = File(...)):
    ...
    try:
        ...
        inserted, skipped = ingest_xlsx(tmp_path, verbose=False)

        # Get summary of what was ingested
        df = pd.read_excel(tmp_path)
        df['gameDate'] = pd.to_datetime(df['gameDate'])

        name_col = next((c for c in ['fullName','pitcher','pitcherName'] if c in df.columns), None)
        summary = []
        if name_col:
            for player, grp in df.groupby(name_col):
                games = grp['gameDate'].dt.date.nunique()
                pitches = len(grp)
                summary.append({
                    "player": str(player),
                    "games": games,
...
async def generate_from_db(
    ...
    try:
        ...
        df = pd.read_sql(
            """
            SELECT * FROM pitches
            WHERE pitcher_name = %s AND game_date = %s
            """,
        ...
        if df.empty:
            return JSONResponse({"error": "No data found for that player and date"}, status_code=404)

        # Remap DB columns back to Trackman column names
        col_map = {
        ...
        # Save as temp xlsx for build_report
