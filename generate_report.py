...
def fetch_player_headshot(player_id, level='MLB'):
    """Try MLB headshot first, then MiLB, then generic fallback."""
    # 1. Try MLB action/headshot
    mlb_url = (f"https://img.mlbstatic.com/mlb-photos/image/upload/"
               f"d_people:generic:headshot:67:current.png/"
               f"w_213,q_auto:best/v1/people/{player_id}/headshot/67/current")
    path = _fetch_url(mlb_url)
    if path:
        return path
    # 2. Try MiLB headshot
    milb_url = (f"https://img.mlbstatic.com/mlb-photos/image/upload/"
                f"w_180,g_auto,c_fill/v1/people/{player_id}/headshot/milb/current")
    path = _fetch_url(milb_url)
    if path:
        return path
    # 3. Try MLB stats API to get a fresh photo URL
    try:
        import json, urllib.request
        api_url = f"https://statsapi.mlb.com/api/v1/people/{player_id}?fields=people,id,fullName,currentAge"
        with urllib.request.urlopen(api_url, timeout=3) as r:
            pass  # just confirm player exists
        # Try alternate MLB photo endpoint
        alt_url = f"https://securea.mlb.com/mlb/images/players/head_shot/{player_id}.jpg"
        path = _fetch_url(alt_url, suffix='.jpg')
        if path:
            return path
    except Exception:
        pass
...
def fetch_team_logo(team_id):
    """Try ESPN MLB logo, then MLB static, then MiLB static."""
    tid = int(team_id)
    # 1. Try ESPN MLB logo (works for all 30 MLB teams)
    espn_abbr = _team_abbr_espn(tid)
    if espn_abbr != str(tid):  # we have a known mapping
        espn_url = f"https://a.espncdn.com/combiner/i?img=/i/teamlogos/mlb/500/{espn_abbr}.png&w=120&h=120&transparent=true"
        path = _fetch_url(espn_url, suffix='.png')
        if path:
            return path
    # 2. Try MLB static cap logo
    cap_url = f"https://www.mlbstatic.com/team-logos/team-cap-on-dark/{tid}.svg"
    path = _fetch_url(cap_url, suffix='.svg')
    if path:
        return path
    # 3. Try MLB stats API team logo (covers MiLB affiliates)
    mlb_logo = f"https://www.mlbstatic.com/team-logos/{tid}.svg"
    path = _fetch_url(mlb_logo, suffix='.svg')
    if path:
        return path
    # 4. Try MLB photos API for team logo
    milb_logo = f"https://img.mlbstatic.com/mlb-photos/image/upload/w_120,q_auto/v1/org/{tid}/logo/spot/current"
    path = _fetch_url(milb_logo, suffix='.png')
    return path
...
# Trackman team abbreviation -> MLB team ID
_TEAM_ABBR_TO_ID = {
    ...
    # common alternates
    'WAS':120,'WSN':120,'CHS':145,'CHW':145,'SDP':135,'SFG':137,
    'KCR':118,'TBR':139,'NYM':121,'NYY':147,
}

def _team_id_from_abbr(abbr):
    if not abbr:
        return None
    return _TEAM_ABBR_TO_ID.get(str(abbr).strip().upper())
...
# ── MLB Stats API — Game Line helpers ────────────────────────────────────────
...
def _rate_stat(n, d, dec=2):
    return round(n / d, dec) if d else 0.0

def _fip_calc(k, bb, hr, ip, c=3.10):
    return round(((13 * hr + 3 * bb - 2 * k) / ip) + c, 2) if ip else 0.0

def _fmt_ip_dec(ip_dec):
    f = int(ip_dec)
    return f"{f}.{round((ip_dec - f) * 3)}"

def _extract_mlb_stats(s):
    ...
    return {'ip': ip, 'h': h, 'r': r_, 'er': er, 'bb': bb, 'k': k, 'hr': hr,
            'era':  _rate_stat(er * 9, ip),
            'whip': _rate_stat(h + bb, ip),
            'k9':   _rate_stat(k * 9,  ip),
            'bb9':  _rate_stat(bb * 9, ip),
            'fip':  _fip_calc(k, bb, hr, ip)}

def _empty_mlb_stats():
    return {x: 0   for x in ['ip','h','r','er','bb','k','hr']} | \
           {x: 0.0 for x in ['era','whip','k9','bb9','fip']}

def fetch_game_line(pitcher_id, game_date_str, level='MLB'):
    """
    Fetch last outing stats + season totals from MLB Stats API.
    Returns (outing_dict, season_dict) or (None, None) on failure.
    """
    SPORT_IDS = {'MLB': 1, 'AAA': 11, 'AA': 12,
                 'HIGH-A': 13, 'HIGH A': 13, 'A': 14}
    sport_id = SPORT_IDS.get(str(level).upper().strip(), 1)
    BASE     = 'https://statsapi.mlb.com/api/v1'
    season   = game_date_str[:4]
    hdrs     = {'User-Agent': 'Mozilla/5.0 (PitchingWRX/1.0)'}

    try:
        import json as _json

        sched_url = (f"{BASE}/schedule?sportId={sport_id}&date={game_date_str}"
                     f"&gameType=R,S,E&hydrate=team")
        req = urllib.request.Request(sched_url, headers=hdrs)
        with urllib.request.urlopen(req, timeout=10) as r:
            sched = _json.loads(r.read())
        ...
        for de in sched.get('dates', []):
            for g in de.get('games', []):
                ...
                req2 = urllib.request.Request(
                    f"{BASE}/game/{gk}/boxscore", headers=hdrs)
                with urllib.request.urlopen(req2, timeout=10) as r2:
                    box = _json.loads(r2.read())
                for side in ('home', 'away'):
                    td = box.get('teams', {}).get(side, {})
                    for _, pd_ in td.get('players', {}).items():
                        if pd_.get('person', {}).get('id') == int(pitcher_id):
                            stats = pd_.get('stats', {}).get('pitching', {})
                            if _parse_ip(stats.get('inningsPitched', '0')) > 0:
                                outing = _extract_mlb_stats(stats)
                                break
                    if outing:
                        break
                if outing:
        ...
        szn_url = (f"{BASE}/people/{pitcher_id}/stats"
                   f"?stats=season&group=pitching&sportId={sport_id}"
                   f"&season={season}&gameType=R")
        req3 = urllib.request.Request(szn_url, headers=hdrs)
        with urllib.request.urlopen(req3, timeout=10) as r3:
            szn_data = _json.loads(r3.read())

        season_stats = None
        for sg in szn_data.get('stats', []):
            for sp in sg.get('splits', []):
                season_stats = _extract_mlb_stats(sp.get('stat', {}))
                break
            if season_stats:
                break

        return (outing or _empty_mlb_stats(),
                season_stats or _empty_mlb_stats())

    except Exception as e:
        print(f'  game_line fetch failed: {e}')
        return None, None


def build_game_line_table(outing, season, pitcher_name, game_date_str, opponent, role):
    """
    Returns a list of ReportLab flowables showing Last Outing | Season Totals.
    Page usable width = 8.5 - 0.25 - 0.25 = 8.0 inches.
    """
    _TXT_WHT = '#E8EAF0'
    _TXT_GR  = '#8A90A8'
    _TXT_ORG = '#F47920'
    ...
    def _rl(h): return colors.HexColor(h)

    def _p(txt, size=7, color=_TXT_WHT, bold=False, align=TA_CENTER):
        ps = ParagraphStyle('_gl',
            fontName='Helvetica-Bold' if bold else 'Helvetica',
            fontSize=size, textColor=_rl(color),
            alignment=align, leading=size + 2)
        return Paragraph(txt, ps)

    def sc(val, is_int=False, dec=2, alert=False, good=False):
        if is_int:
            txt = str(int(val))
        elif dec == -1:
            txt = _fmt_ip_dec(val)
        else:
            txt = f"{val:.{dec}f}" if isinstance(val, float) else str(val)
        color = _TXT_RED if alert else (_TXT_GRN if good else _TXT_WHT)
        content = f'<b>{txt}</b>' if (alert or good) else txt
        return _p(content, size=7, color=color)

    era_bad  = outing['era']  > season['era']  + 1.0 and season['era']  > 0
    fip_bad  = outing['fip']  > season['fip']  + 1.0 and season['fip']  > 0
    era_good = not era_bad and outing['era'] < 3.00 and outing['ip'] >= 1.0
    fip_good = not fip_bad and outing['fip'] < 3.50 and outing['ip'] >= 1.0

    try:
        d_fmt = datetime.date.fromisoformat(game_date_str).strftime('%m/%d/%y')
    except:
        d_fmt = game_date_str

    role_col  = _TXT_ORG if role == 'SP' else _TXT_GR
    opp_short = (str(opponent)[:22] + '…') if len(str(opponent)) > 23 else str(opponent)

    n_info = 3
    n_stat = 11

    grp = [_p('', size=6)] * n_info
    grp += [_p('<b>| Last Outing</b>',   size=7.5, color=_TXT_ORG, bold=True, align=TA_LEFT)]
    grp += [_p('', size=6)] * (n_stat - 1)
    grp += [_p('', size=6)]
    grp += [_p('<b>| Season Totals</b>', size=7.5, color=_TXT_ORG, bold=True, align=TA_LEFT)]
    grp += [_p('', size=6)] * (n_stat - 1)

    labels = ['Pitcher / Opponent','Date','Role',
              'IP','H','R','ER','BB','K','ERA','WHIP','K/9','BB/9','FIP',
              '',
              'IP','H','R','ER','BB','K','ERA','WHIP','K/9','BB/9','FIP']
    hdr = [_p(f'<b>{lbl}</b>', size=5.5, color=_TXT_GR,
              align=TA_LEFT if i < n_info else TA_RIGHT)
           for i, lbl in enumerate(labels)]

    name_ps = ParagraphStyle('_nl', fontName='Helvetica-Bold', fontSize=8,
                              textColor=_rl(_TXT_WHT), leading=12)
    name_cell = Paragraph(
        f'<b>{pitcher_name}</b><br/>'
        f'<font color="{_TXT_GR}" size="6.5">  vs {opp_short}</font>',
        name_ps)

    o, s = outing, season
    data = [
        name_cell,
        _p(f'<b>{d_fmt}</b>', size=7, color=_TXT_ORG, bold=True),
        _p(f'<b>{role}</b>',  size=7, color=role_col,  bold=True),
        sc(o['ip'],  dec=-1),
        sc(o['h'],   is_int=True),
        sc(o['r'],   is_int=True),
        sc(o['er'],  is_int=True),
        sc(o['bb'],  is_int=True),
        sc(o['k'],   is_int=True),
        sc(o['era'], alert=era_bad,        good=era_good),
        sc(o['whip'],alert=o['whip']>1.5),
        sc(o['k9'],  good=o['k9']>10.0),
        sc(o['bb9'], alert=o['bb9']>4.0),
        sc(o['fip'], alert=fip_bad,        good=fip_good),
        _p(''),
        sc(s['ip'],  dec=-1),
        sc(s['h'],   is_int=True),
        sc(s['r'],   is_int=True),
    ...
    # Total usable width = 8.0 inches (8.5 - 0.25 margins each side)
    # info: 1.30 + 0.44 + 0.30 = 2.04
    # sep:  0.05
    # stats x2: 11 cols * ~0.268 each = 2.948 x2 = 5.896
    # sep + 2x stats + info = 0.05 + 5.896 + 2.04 = 7.986  ✓ under 8.0
    W = inch
    info_w = [1.28*W, 0.43*W, 0.29*W]
    stat_w = [0.26*W, 0.19*W, 0.19*W, 0.21*W, 0.21*W, 0.19*W,
              0.31*W, 0.35*W, 0.35*W, 0.31*W, 0.30*W]
    sep_w  = [0.05*W]
    col_widths = info_w + stat_w + sep_w + stat_w

    o_s = n_info;   o_e = n_info + n_stat - 1
    sep = o_e + 1
    s_s = sep + 1;  s_e = s_s + n_stat - 1

    rows = [grp, hdr, data]
    t = Table(rows, colWidths=col_widths, rowHeights=[16, 14, 30])
    t.setStyle(TableStyle([
        ('BACKGROUND',    (0,0),    (-1,0),    _rl(_HDR_BG)),
        ('SPAN',          (o_s,0),  (o_e,0)),
        ('SPAN',          (s_s,0),  (s_e,0)),
        ...
        ('BACKGROUND',    (0,1),    (-1,1),    _rl(_HDR_BG)),
        ('LEFTPADDING',   (0,1),    (-1,1),    2),
        ('RIGHTPADDING',  (0,1),    (-1,1),    2),
        ('TOPPADDING',    (0,1),    (-1,1),    3),
        ('BOTTOMPADDING', (0,1),    (-1,1),    3),
        ('ALIGN',         (n_info,1),(s_e,1),  'RIGHT'),
        ('BACKGROUND',    (0,2),    (-1,2),    _rl(_ROW_BG)),
        ('LEFTPADDING',   (0,2),    (-1,2),    2),
        ('RIGHTPADDING',  (0,2),    (-1,2),    2),
        ('TOPPADDING',    (0,2),    (-1,2),    4),
        ('BOTTOMPADDING', (0,2),    (-1,2),    4),
        ('ALIGN',         (n_info,2),(s_e,2),  'RIGHT'),
        ('VALIGN',        (0,0),    (-1,-1),   'MIDDLE'),
        ('BACKGROUND',    (sep,0),  (sep,-1),  _rl(_SEP_BG)),
        ('LINEBELOW',     (0,2),    (-1,2),    0.5, _rl('#3A4060')),
    ]))
...
def _prepare_df(df):
    """Shared prep logic for both file-loaded and DB-loaded DataFrames."""
    if 'count' in df.columns:
        df['count_fixed'] = df['count'].apply(fix_count)
    elif 'count_fixed' not in df.columns:
        df['count_fixed'] = ''
    if 'pitchResult' in df.columns:
        ...
        df['is_whiff'] = df.get('is_whiff', 0).astype(bool)
        df['is_called_strike'] = df.get('is_called_strike', 0).astype(bool)
        df['is_foul'] = False
    if 'pitchOutcome' in df.columns:
        ...
        df['is_swing']   = df.get('is_swing', 0).astype(bool)
        df['is_in_play'] = False
        df['is_strike']  = df.get('is_strike', 0).astype(bool)
    if 'ExitVel' in df.columns:
        df['ExitVel_num'] = pd.to_numeric(df['ExitVel'].replace('-', np.nan), errors='coerce')
    else:
        df['ExitVel_num'] = np.nan
    ...
    # Coordinate flip for catcher view
    if 'x' in df.columns:
        df['px'] = -df['x']
    if 'y' in df.columns:
        df['pz'] = df['y'] + 2.5
...
def build_report(data_path, logo_path, output_path,
                 ...
                 df_override=None):
    df = _prepare_df(df_override.copy()) if df_override is not None else load_data(data_path)
    ...
    if use_db:
        try:
            # Only ingest if we have a file path (not when called from DB directly)
            ...
            pitcher_id=int(df['pitcherId'].iloc[0]) if 'pitcherId' in df.columns and pd.notna(df['pitcherId'].iloc[0]) else None
            pitcher_name=str(df['fullName'].iloc[0])
            ...
            game_log_df=get_game_log(pitcher_id=pitcher_id,
                                     pitcher_name=pitcher_name if not pitcher_id else None)
            print(f" DB: szn_df={len(szn_df)} rows, game_log={len(game_log_df)} rows")
        except Exception as e:
    ...
    if not team_id and 'team' in df.columns:
        team_id = _team_id_from_abbr(df['team'].iloc[0])
    ...
    # ── Game line: Last Outing | Season Totals ──
    _player_id_val = (int(df['pitcherId'].iloc[0])
                      if 'pitcherId' in df.columns and pd.notna(df['pitcherId'].iloc[0])
                      else None)
    if _player_id_val:
        try:
            _gl_date  = game_date if game_date else pd.to_datetime(df['gameDate'].iloc[0]).strftime('%Y-%m-%d')
            _gl_level = str(df['level'].iloc[0]) if 'level' in df.columns else 'MLB'
            _gl_opp   = str(df['opponent'].iloc[0]) if 'opponent' in df.columns else ''
            _gl_ip    = df['inn'].nunique() if 'inn' in df.columns else 0
            _gl_role  = 'SP' if _gl_ip >= 3 else 'RP'
            _gl_out, _gl_szn = fetch_game_line(_player_id_val, _gl_date, _gl_level)
            if _gl_out and _gl_szn:
                story.extend(build_game_line_table(
                    _gl_out, _gl_szn, name, _gl_date, _gl_opp, _gl_role))
        except Exception as _gl_e:
            print(f'  game line skipped: {_gl_e}')
    ...
    story.append(orange_hr(6))
    log_cols_all=[('pitchNumInGame','#'),('inn','Inn'),('outs','Outs'),('count_fixed','Count'),
                  ('batterAbbrevName','Batter'),('batterHand','Hand'),('pitchTypeFull','Pitch Type'),
                  ('Vel','Velo'),('Spin','Spin'),('IndVertBrk','IVB'),('HorzBrk','HB'),
                  ('pitchResult','Result'),('ExitVel','EV')]
    log_cols=[c for c,_ in log_cols_all if c in df.columns]
    log_labels=[l for c,l in log_cols_all if c in df.columns]
...
def build_report_from_db(player_name, game_date, logo_path, output_path,
    ...
    # Pass DataFrame directly -- no temp file needed
    build_report(
        data_path=None,
        logo_path=logo_path,
