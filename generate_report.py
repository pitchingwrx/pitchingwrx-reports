...
‘’’
Monthly-average trend charts per pitch type.
Annotates each data point with level badge (MLB / AAA / EXB).
Gaps > 60 days break the line so offseason shows as empty space.
‘’’
...
df_db=pd.read_sql(f’’‘SELECT batter_hand,pitch_type_full,is_swing,is_whiff,
is_called_strike,exit_vel,launch_ang,pitch_result,count_str
FROM pitches WHERE {where}’’’,conn,params=params)
conn.close()
...
if db_path and os.path.exists(db_path):
    try:
        ingest_xlsx(data_path, db_path=db_path, verbose=False)
        game_date_str=pd.to_datetime(df['gameDate'].iloc[0]).strftime('%Y-%m-%d')
        ...
        szn_df=season_averages(pitcher_id=pitcher_id,
                               ...
                               before_date=game_date_str, db_path=db_path)
