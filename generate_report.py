    ...
    try:
        ...
        for de in sched.get('dates', []):
            for g in de.get('games', []):
                gk = g.get('gamePk')
                req2 = urllib.request.Request(f"{BASE}/game/{gk}/boxscore", headers=hdrs)
        ...
        for sg in szn_data.get('stats', []):
            for sp in sg.get('splits', []):
                ...
                break
            if season_stats: break
        return (outing or _empty_mlb_stats(), season_stats or _empty_mlb_stats())
...
def build_game_line_table(outing, season, pitcher_name, game_date_str, opponent, role):
    """
    Last Outing | Season Totals table in PitchingWRX dark theme.
    Total width = 7.50 inches (well within 8.0 usable).
    All padding kept at 0 to avoid ReportLab negative-width errors.
    """
    _TXT_WHT = '#E8EAF0'
    _TXT_GR  = '#8A90A8'
    _TXT_ORG = '#F47920'
    ...
    opp_short = (str(opponent)[:20] + '…') if len(str(opponent)) > 21 else str(opponent)

    n_info = 3
    n_stat = 11

    grp = [_p('', size=6)] * n_info
    grp += [_p('<b>| Last Outing</b>',   size=7, color=_TXT_ORG, bold=True, align=TA_LEFT)]
    grp += [_p('', size=6)] * (n_stat - 1)
    grp += [_p('', size=6)]
    grp += [_p('<b>| Season Totals</b>', size=7, color=_TXT_ORG, bold=True, align=TA_LEFT)]
    grp += [_p('', size=6)] * (n_stat - 1)
    ...
    name_ps = ParagraphStyle('_nl', fontName='Helvetica-Bold', fontSize=7.5,
                              textColor=_rl(_TXT_WHT), leading=11)
    name_cell = Paragraph(
        f'<b>{pitcher_name}</b><br/>'
        f'<font color="{_TXT_GR}" size="6">vs {opp_short}</font>',
        name_ps)

    o, s = outing, season
    data = [
        name_cell,
        _p(f'<b>{d_fmt}</b>', size=6.5, color=_TXT_ORG, bold=True),
        _p(f'<b>{role}</b>',  size=6.5, color=role_col,  bold=True),
        sc(o['ip'],  dec=-1),
        sc(o['h'],   is_int=True),
        sc(o['r'],   is_int=True),
        sc(o['er'],  is_int=True),
    ...
    # Total = 1.20+0.40+0.28 + 11*(0.255) x2 + 0.04 = 7.50 inches
    W = inch
    info_w = [1.20*W, 0.40*W, 0.28*W]
    stat_w = [0.255*W]*11
    sep_w  = [0.04*W]
    ...
    rows = [grp, hdr, data]
    t = Table(rows, colWidths=col_widths, rowHeights=[15, 13, 28])
    t.setStyle(TableStyle([
        # All cells — zero padding to prevent overflow
