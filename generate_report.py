...
def fetch_game_line(pitcher_id, game_date_str, level='MLB'):
    ...
    try:
        ...
        # ── Last outing: scan box scores on that date ──
...
def build_game_line_table(outing, season, pitcher_name, game_date_str, opponent, role):
    ...
    Styled to match the PitchingWRX dark theme.
    """
    _TXT_WHT = '#E8EAF0'
    _TXT_GR  = '#8A90A8'
    _TXT_ORG = '#F47920'
    ...
    n_info = 3
    n_stat = 11   # IP H R ER BB K ERA WHIP K/9 BB/9 FIP
    ...
    W = inch
    info_w = [1.45*W, 0.50*W, 0.33*W]
    stat_w = [0.29*W, 0.21*W, 0.21*W, 0.24*W, 0.24*W, 0.21*W,
              0.35*W, 0.40*W, 0.40*W, 0.35*W, 0.33*W]
    sep_w  = [0.06*W]
